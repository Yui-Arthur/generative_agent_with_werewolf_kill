...
from .summary_role import summary_role , summary_werewolf , summary_seer , summary_witch , summary_hunter
...
class memory_stream_summary_agent(summary_agent):
    def __init__(self , api_json = "doc/secret/yui.key",  
                 server_url = "140.127.208.185" , agent_name = "Agent1" , room_name = "TESTROOM" , 
                 color = "f9a8d4" , prompt_dir = "doc/prompt/memory_stream"):
        ...
        # init long memory class & models
        self.long_memory : summary_role = None
        # start the game
        self.day = None
        self.turn = 0
    ...
    def __start_game_init__(self , room_data):
        ...
        self.__get_all_role__()
        role_to_class = {
            "werewolf" : summary_werewolf,
            "seer" : summary_seer,
            "witch" : summary_witch,
            "hunter" : summary_hunter,
            "village" : summary_role,
        }
        
        self.long_memory : summary_role = role_to_class[self.role](self.prompt_dir , self.logger, self.api_kwargs)
        if self.role != "werewolf":
            self.long_memory.update_game_info(self.player_name , self.role)
        else:
            self.long_memory.update_game_info(self.player_name , self.role , role_info['game_info']['teamate'])
